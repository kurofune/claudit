...
func runServe(args []string) error {
	...
	fs.Usage = func() {
		out := fs.Output()
		fmt.Fprintln(out, "claudit serve — run a local web daemon that serves a live-updating report.")
		fmt.Fprintln(out)
		fmt.Fprintln(out, "Usage:")
		fmt.Fprintln(out, "  claudit serve [flags]")
		fmt.Fprintln(out)
		fmt.Fprintln(out, "The server binds to 127.0.0.1 by default. The report can contain prompt")
		fmt.Fprintln(out, "text and CWD paths, has no authentication, and is meant for single-user")
		fmt.Fprintln(out, "localhost use only. Exposing it to other hosts is at your own risk.")
		fmt.Fprintln(out)
		fmt.Fprintln(out, "Filter via URL query params (same names as `claudit report` flags):")
		fmt.Fprintln(out, "  /?project=myrepo&last=7d")
		fmt.Fprintln(out, "  /?since=2026-05-01&until=2026-05-15&by=week")
		fmt.Fprintln(out)
