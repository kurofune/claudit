...
- **Interactive mode** (`DJINN_SESSION` unset): the project's always-on instructions (CLAUDE.md / AGENTS.md / the beads rule file) reference this contract. Enforcement is by agent discipline + reviewer convention, not by harness rollback. The audit-trail artifacts — `.djinn/evidence/<bead>.yml` plus the on-disk output of each named procedure the ACTIVE formula declares (`.ac-quality.json`; one review sidecar per declared review loop — `.review-light.json` under the default chain, `.review-impl.json` **and** `.review-tests.json` under `iter-heavy`, none under `iter-rnd`; `.report.json`) — are written under `.djinn/evidence/` and **committed** with the bead's work, so they are permanent and auditable in git history.
- **Single-pass autonomous mode** (`DJINN_SESSION=1`, no formula): `.djinn/PROMPT.md` wraps this contract with harness-specific bits (target-bead matching). The agent writes `.djinn/evidence/<bead>.yml` AND `.djinn/last-status.json`; the harness loads both (`status.Load` → `evidence.Load`, `internal/djinn/agent/accumulator.go`) and then gates the result — the evidence gate compares `len(criteria)` to the bead's `acceptance_criteria` count (`internal/djinn/gate/evidence.go:48`) and the artifacts gate checks each cited path against the diff (`internal/djinn/gate/artifacts.go:55`); a count mismatch, a missing file, or a malformed schema rolls back the iteration (`.djinn/PROMPT.md:38`). The evidence YAML is committed, so this mode's on-disk history matches interactive mode's.
- **Formula autonomous mode** (`DJINN_SESSION=1` with `Cfg.FormulaPath` set — the dominant production path, `internal/djinn/session/run_formula.go`): each contract step runs as its own formula loop step, and the per-step verdicts (ac-quality, the verdict of each review loop the active formula declares — `review-light` under the default chain, `review-impl` / `review-tests` under `iter-heavy` — and verify) live ONLY in the **gitignored** `.djinn/state/<bead>/<step>.iter-N/` forensics dir, with transcript and cost under `.djinn/state/<session>/<bead>/`. Formula steps are explicitly told NOT to write `.djinn/evidence/<bead>.yml` or `.djinn/last-status.json` (`internal/djinn/session/formula_prompts.go` (`BuildImplementPrompt`) — "the harness owns terminal status in formula mode" — with parallel prohibitions in `BuildWriteTestsPrompt` and `BuildRefactorPrompt`). These per-step gate forensics are **EPHEMERAL**: wiped on state cleanup, never committed. What DOES survive into git history is a synthesized `.djinn/evidence/<bead>.yml`: the harness-owned `commit-push` step reads the verify-loop's report and commits a per-criterion evidence artifact (one row per acceptance criterion, status transcribed from the report) BEFORE the push, so it rides the pushed commit (`internal/djinn/session/formula_evidence.go`, `emitFormulaEvidence`, djinn-id2a). This is the single writer of the formula-mode artifact — the per-step prohibition above is preserved unchanged, so terminal-status ownership stays with the harness and there is no double-write. A past formula run is therefore auditable from git alone (close reason + the committed evidence YAML) even after `.djinn/state/` is cleaned; only the finer-grained per-step gate verdicts (the ac-quality / review sidecars) remain ephemeral.
...
1. **Tighten the bead's acceptance_criteria.** Apply each `rewrite_hint` via `bd update <bead_id> --acceptance="<rewritten criteria>"` — preserve unflagged criteria in their original order so the segment count still aligns with the evidence YAML you write in §6. Re-invoke the AC-quality review until verdict is `CONCRETE`. Then proceed to §3.
...
|---|---|---|
| `core/iter.formula.toml` (the default), `core/iter-light.formula.toml` | `review-light-loop` | ONE single-lens review pass |
| `core/iter-tdd.formula.toml` | `review-light-loop` | ONE single-lens review pass |
| `core/iter-heavy.formula.toml` | `review-impl-loop`, `review-tests-loop` | the implementation review AND the tests review |
| `core/iter-rnd.formula.toml` | none | no review — see the R&D exemption below |

Run the surface the formula declares, neither heavier nor lighter. Running two 5-panel orchestrations where the formula declares one single-lens pass reinstates roughly a dozen review agents the default chain exists to cut; running one where the formula declares two drops a review the bead was deliberately routed for. In interactive mode the active formula is the one your adapter's orchestrator resolves for the bead (its `formula:<name>` label, else the embedded default); when no formula is in play at all, the embedded default's review surface is the baseline. **`core/iter-rnd.formula.toml` declaring no review loop is the one documented exemption** — an explicit routing decision recorded when the bead was labeled, producing knowingly unreviewed spike code. It is not a licence to skip review on your own judgement under any other formula.
...
Before declaring complete you MUST write `.djinn/evidence/{bead_id}.yml` — typed YAML with one `criteria` entry per acceptance criterion. This file is the mechanical audit trail that converts self-reported completion into a checkable claim. In formula mode you do NOT write this file: the harness's `commit-push` step synthesizes `.djinn/evidence/<bead_id>.yml` from the verify report.

**You MUST:**

- [ ] Write the file to disk. Do NOT print YAML to stdout, do NOT wrap it in a markdown fence — the audit reads from disk, not from your transcript.
- [ ] Write to path `.djinn/evidence/<bead_id>.yml` (extension `.yml`, not `.md`). The filename `<bead_id>` MUST equal the bead you claimed in §1.
...
Provide three inputs: the `bead_id` you claimed in §1, the diff base ref (use the harness `pre_sha` in autonomous mode; otherwise `HEAD~N` where N is this iteration's commit count), and the evidence file path `.djinn/evidence/{bead_id}.yml`.
...
Commit any remaining changes after steps 4, 5, 6, and 7 fixes. Include the evidence file in the commit.
